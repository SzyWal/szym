window.addEventListener('load', () => {
    ...
    function renderFrame() {
        if (running) {
            ...
            if (snake.filter(square => square.x === getHead() && square.y === getHead()).length >= 2) {
                setDefault();
            } else {

                if (apple.x === getHead() && apple.y === getHead().y) {
                    points++;
    ...
    function generateAppleLocation() {
        ...
        } while (snake.filter(square => square.x === location.x && square.y).length > 0);
        return location;
    }
